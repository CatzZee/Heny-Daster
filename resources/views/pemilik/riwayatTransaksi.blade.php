    ...
    @push('styles')
        ...
        <style>
            ...
            .transaction-card {
                ...
                /* Invisible border for consistency */
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="search" class="form-control border-start-0 ps-0"
                                    placeholder="ID Transaksi, Nama Pembeli..." value="{{ request('search') }}">
                            </div>
                        </div>
            ...
            <div class="mb-2 mb-md-0">
                ...
                <select class="form-select form-select-sm d-inline-block w-auto mx-1"
                    ...
                    @foreach ([5, 10, 15, 25] as $num)
                        <option value="{{ request()->fullUrlWithQuery(['per_page' => $num]) }}"
                            {{ request('per_page', 10) == $num ? 'selected' : '' }}>{{ $num }}</option>
                    @endforeach
                </select>
                data per halaman
            </div>
                    ...
                    <div class="card-header-custom" onclick="toggleDetails('{{ $transaksi->id }}')">
                        <div class="row align-items-center">
                            ...
                            <div class="col-6 col-md-3 mb-2 mb-md-0">
                                ...
                                <span
                                    class="text-dark">{{ \Carbon\Carbon::parse($transaksi->waktu_transaksi)->isoFormat('D MMM Y, HH:mm') }}</span>
                            </div>
                                ...
                                <table class="table table-details table-borderless">
                                            ...
                                            <tr class="border-bottom">
                                                <td>
                                                    <div class="fw-semibold">
                                                        {{ $d->produk->nama_produk ?? 'Produk Dihapus' }}</div>
                                                    <small class="text-muted">{{ $d->produk->kode_produk ?? '-' }}</small>
                                                </td>
                                                <td class="text-center"><span
                                                        class="badge bg-light text-dark border">{{ $d->produk->ukuran_baju ?? '-' }}</span>
                                                </td>
                                                <td class="text-center">{{ $d->jumlah }}</td>
                                                <td class="text-end">Rp
                                                    {{ number_format($d->harga_saat_transaksi, 0, ',', '.') }}</td>
                                                <td class="text-end fw-semibold">Rp
                                                    {{ number_format($d->harga_saat_transaksi * $d->jumlah, 0, ',', '.') }}
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                    <tfoot>
                                        <tr>
                                            ...
                                            <td class="text-end pt-3 fw-bold fs-5 text-dark">Rp
                                                {{ number_format($transaksi->total_harga, 0, ',', '.') }}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" class="text-end pt-3 text-muted">
                                                <form
                                                    onsubmit="return confirm('Apakah Anda yakin ingin membatalkan/menghapus transaksi ini?');"
                                                    action="{{ route($routePrefix . '.riwayat-transaksi.destroy', $transaksi->id) }}"
                                            ...
                                            <td class="text-end pt-3 fw-bold fs-5 text-dark">
                ...
                <nav>
                    <div class="d-flex gap-1">
                        ...
                        @if ($transaksis->onFirstPage())
                            <span class="page-link-custom text-muted" style="cursor: not-allowed; opacity: 0.6;"><i
                                    class="bi bi-chevron-left"></i></span>
                        @else
                            <a href="{{ $transaksis->previousPageUrl() }}" class="page-link-custom"><i
                        ...
                        @if ($transaksis->hasMorePages())
                            <a href="{{ $transaksis->nextPageUrl() }}" class="page-link-custom"><i
